...
void thread_compress(Image_t* image, work_package_t* work_packages, char **filteredData, z_stream* zStreams, uint32_t* adlerSums, char **deflateOutput, size_t* deflateOutputSize, char** filteredChunks, int threadID)
{
	...
	char *deflate_stream = deflate_stream_start;//open_memstream(&deflateOutput[threadID], &deflateOutputSize[threadID]);
	...
	do {
		...
		memcpy(deflate_stream, &output_buffer, have);
		deflate_stream += have;
		deflateOutputSize[threadID] += have;
