...
bool comparePNG(Image_t* image1, Image_t* image2)
{
	...
	pixel32bpp pixelIn, pixeCompare;
	for (int i = 0; i < image1->height; i++)
		for (int j = 0; j < image1->width; j++) {
			pixelIn = ((pixel32bpp*)image1->row_pointers[i])[j];
			pixeCompare = ((pixel32bpp*)image2->row_pointers[i])[j];
			if (pixelIn.a != pixeCompare.a || pixelIn.r != pixeCompare.r || pixelIn.g != pixeCompare.g || pixelIn.b != pixeCompare.b) {
				std::cout << "Diferencia de pixels en: " << i << " " << j << std::endl;
				std::cout << "\t CPU: r-> " << int(image1->row_pointers[i][j * 4]) << " g-> " << int(image1->row_pointers[i][j * 4 + 1]) << " b-> " << int(image1->row_pointers[i][j * 4 + 2]) << " a-> " << int(image1->row_pointers[i][j * 4 + 3]) << std::endl;
				std::cout << "\t GPU:" << int(image2->row_pointers[i][j * 4]) << std::endl;
				igual = false;
			}
		}
